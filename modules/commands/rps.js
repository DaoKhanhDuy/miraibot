module.exports.config = {
  ...
  hasPermssion: 0,
  credits: "DuyVuongUwU",
  description: "Game oẳn tù tì nhiều người...",
...
module.exports.run = function({ event, api, args, getText, global }) {
  ...
  if (args[0] == 'create') {
    ...
    global.rps.set(threadID, { author: senderID, start: false, max: args[1], player: [senderID], choose: [],  username: [], win: [], lose: [], draw: [] });
  ...
  if (args[0] == 'join') {
    ...
    if (gameThread.player.length == gameThread.max) {
      ...
      var list = []
      for (var ids of gameThread.player) { 
        var name = (await User.getData(ids)).name;
        gameThread.username[ids] = name;
      }
      setTimeout(() => {
