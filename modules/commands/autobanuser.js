	...
	if ((global.client.autoban[senderID].timeStart + thoi_gian_spam) <= Date.now()) {
	  ...
	  if (global.client.autoban[senderID].number >= so_lan_spam) {
	    const time = moment.tz("Asia/Ho_Chi_minh").format("DD/MM/YYYY HH:mm:ss");
			...
			data.banned = true;
			data.reason = `spam bot ${so_lan_spam} lần/${thoi_gian_spam/10}phút`;
			data.autoban = {
			  timeStart: Date.now(),
			  unbanAfter
			};
  		...
  		api.sendMessage({
  		  body: senderID + " | " + dataUser.name + `\n🤍 Bạn đã bị cấm sử dụng bot=)) ${unbanAfter/6000000}phút với lý do: spam bot \n🖤 Auto unban sau ${Math.floor(unbanAfter/6000000)}phút`,
